...
item_regex="DA_DID_Reputation_BlackbeardSign_0[234]"

command -v jq >/dev/null 2>&1 || { echo "jq is required to build ship-token override PAK." >&2; exit 1; }
...
if [ "$changed_tables" -eq 0 ]; then
  echo "No ship faction token loot entries matched; refusing to build empty PAK." >&2
  exit 1
fi
...
echo "Built $out_pak with $changed_entries ship faction token entries across $changed_tables tables at 2x."
EOF
...
- `WindrosePlus_ShipFactionTokens_2x_P.pak`: custom server-side PAK that doubles Blackbeard faction ship-token drops (`DA_DID_Reputation_BlackbeardSign_02/03/04`).
